...
- **CI actually runs now** — the workflow triggered on `main` but the default branch is `master`, so no CI job had ever executed. Also: test matrix installs `sklearn` (required at collection by scan-test fixtures) and the `importer` extra, `fail-fast` disabled, the never-adopted `ruff format --check` step removed (style enforcement is `ruff check`, per convention), and the uncalibrated coverage gate dropped pending calibration.
