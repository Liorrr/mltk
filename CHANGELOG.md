...
### Added
- **Whole-word homoglyph detection in `assert_no_unicode_attacks`** — the homoglyph check now catches single-script spoofs (`раура`, fullwidth `ｐａｙｐａｌ`) in addition to mixed-script tokens (`pаypal`), and Cyrillic/fullwidth/mathematical-alphanumeric letters all count as mixed-script partners of ASCII Latin. Two evidence rules, because they are not equally self-evident. **Mixed-script** requires the mixture inside one *word* — a run of alphanumerics ending at punctuation **or at a change of script family** — since a token is `\S+`: that clears CJK prose carrying a fullwidth acronym beside an ASCII word (`設定はＯＮにしてWiFiを有効化する`), `f(𝐱)`, and URLs with a Cyrillic path segment, while still catching `pаypal` embedded in space-free Japanese, Chinese, Korean or Thai. Mathematical alphanumerics count toward it only three-or-more to a word, so `𝐀x` (a bold matrix applied to a variable) is notation while `pay𝐩𝐚𝐥` is a spoof. **Single-script** is resolved against script context the way UTS #39 defines whole-script confusables, because "every letter is a Latin twin" is also true of ordinary words (МОСКВА, СССР, хор, και): new `single_script_spoofs` parameter on both `detect_unicode_attacks` and `assert_no_unicode_attacks` takes `"auto"` (default — flag only where the surrounding letters, on the same line, are predominantly Latin), `"always"` (Latin-only corpora with no mathematics) or `"never"` (mixed-script rule alone). A token made *entirely* of mathematical alphanumerics is opt-in under `"always"`: U+1D400–U+1D7FF has no native prose, so Latin context is guaranteed by construction and the gate cannot tell `𝐚𝐛𝐜` from `𝐩𝐚𝐲𝐩𝐚𝐥`. Homoglyph findings now carry `reason: "mixed_script" | "single_script"`. Greek stays excluded from the mixed-script rule so `α-helix`, `5μm` and `kΩ` remain clean.
- **`assert_chrf` — chrF / chrF++ generation quality** (`mltk.domains.nlp.generation`) — corpus chrF (Popović 2015) via optional `sacrebleu` (`mlspec[nlp]`), reported in `[0, 1]`. `word_order=2` selects chrF++ and names the result `nlp.chrfpp` so the two stay distinguishable downstream. An empty corpus fails rather than scoring 1.0. `assert_bleu` gains the same empty-corpus guard, which previously raised `ZeroDivisionError` out of nltk, and completes the S102 `require_same_length` sweep for `nlp.generation` — mismatched lengths now raise `ValueError` instead of truncating to the shorter list.
