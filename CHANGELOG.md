...
- **`assert_no_drift` KL and Wasserstein paths route through `mltk._rust`** — `_drift_kl`'s inline histogram math was byte-identical to `mltk._rust.kl_divergence` and now delegates to it (Rust acceleration, zero behavior change); `_drift_wasserstein` imported scipy directly and hard-failed without it, now uses `mltk._rust.wasserstein` (Rust → scipy → numpy fallback chain). JS deliberately stays inline: `_drift_js` reports unnormalized nats while `mltk._rust.js_divergence` normalizes by ln 2 — switching would silently rescale user thresholds by ~1.44×; the scale is now documented in the docstring.
