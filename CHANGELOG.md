...
- **Documented env/yaml config fields are now consumed** — `drift_method`/`drift_threshold` (assert_no_drift defaults), `report_dir` (pytest-plugin HTML report), and `pii_patterns` (assert_no_pii default categories) apply when — and only when — the user actually set them; unset config leaves legacy defaults untouched (per-method drift threshold table, all PII patterns). `report_format`/`baseline_dir`/`seed` remain unwired (tracked in BACKLOG).
