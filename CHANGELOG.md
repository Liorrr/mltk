...
### Added
- **Whole-word homoglyph detection in `assert_no_unicode_attacks`** — the homoglyph check now catches single-script spoofs (`раура`, fullwidth `ｐａｙｐａｌ`) in addition to mixed-script tokens (`pаypal`), and Cyrillic/fullwidth/mathematical-alphanumeric letters all count as mixed-script partners of ASCII Latin. Two evidence rules, because they are not equally self-evident. **Mixed-script** requires the mixture inside one word — one alphanumeric run whose letters are all Latin or a partner — since a token is `\S+` and CJK writes no spaces, so a whole Japanese sentence carrying a fullwidth acronym beside an ASCII word (`設定はＯＮにしてWiFiを有効化する`) arrives as one token and is not a spoof; this also clears `f(𝐱)` and URLs with a Cyrillic path segment. **Single-script** is resolved against script context the way UTS #39 defines whole-script confusables, because "every letter is a Latin twin" is also true of ordinary words (МОСКВА, СССР, хор, και): new `single_script_spoofs` parameter on both `detect_unicode_attacks` and `assert_no_unicode_attacks` takes `"auto"` (default — flag only where the surrounding letters, on the same line, are predominantly Latin), `"always"` (Latin-only corpora) or `"never"` (mixed-script rule alone). Mathematical alphanumerics are **opt-in under `"always"`**: U+1D400–U+1D7FF has no native prose, so Latin context is guaranteed by construction and the gate cannot tell `𝐚𝐛𝐜` from `𝐩𝐚𝐲𝐩𝐚𝐥` — under `"auto"` a pure math-bold spoof is missed, while `pay𝐩𝐚𝐥` is still caught by the mixed-script rule. Homoglyph findings now carry `reason: "mixed_script" | "single_script"`. Greek stays excluded from the mixed-script rule so `α-helix`, `5μm` and `kΩ` remain clean.
