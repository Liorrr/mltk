...
### Added
- **Whole-word homoglyph detection in `assert_no_unicode_attacks`** — the homoglyph check now catches single-script spoofs (`раура`, fullwidth `ｐａｙｐａｌ`, mathematical-bold `𝐩𝐚𝐲𝐩𝐚𝐥`) in addition to mixed-script tokens (`pаypal`), and Cyrillic/fullwidth/mathematical-alphanumeric letters all count as mixed-script partners of ASCII Latin. Because "every letter is a Latin twin" is also true of ordinary words (МОСКВА, СССР, хор, και), the whole-word rule is resolved against script context the way UTS #39 defines whole-script confusables: new `single_script_spoofs` parameter on both `detect_unicode_attacks` and `assert_no_unicode_attacks` takes `"auto"` (default — flag only where the surrounding letters, on the same line, are predominantly Latin), `"always"` (Latin-only corpora) or `"never"` (mixed-script rule alone). Homoglyph findings now carry `reason: "mixed_script" | "single_script"`. Greek stays excluded from the mixed-script rule so `α-helix`, `5μm` and `kΩ` remain clean.
