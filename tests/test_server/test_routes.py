...
def client(tmp_path):
    ...
    # EXPECTED: each test starts with an empty database
...
def test_submit_run(client):
    # SCENARIO: POST a valid run payload to /api/runs
...
def test_list_runs(client):
    ...
    client.post("/api/runs", json={"project": "proj", "results": SAMPLE_RESULTS[:1]})
    client.post("/api/runs", json={"project": "proj", "results": SAMPLE_RESULTS})
...
def test_get_run(client):
    ...
    post_resp = client.post("/api/runs", json={"project": "detail_proj", "results": SAMPLE_RESULTS})
...
def test_trends(client):
    ...
    for _ in range(3):
        client.post("/api/runs", json={"project": "trend_proj", "results": SAMPLE_RESULTS})

    resp = client.get("/api/trends/trend_proj")
    assert resp.status_code == 200
    body = resp.json()
...
def test_list_runs_project_filter(client):
    ...
    client.post("/api/runs", json={"project": "alpha", "results": SAMPLE_RESULTS[:1]})
    client.post("/api/runs", json={"project": "beta", "results": SAMPLE_RESULTS})

    resp = client.get("/api/runs?project=alpha")
    assert resp.status_code == 200
    body = resp.json()
