...
class TestNDCG:
    ...
    def test_empty_queries_fail_by_default(self) -> None:
        # SCENARIO: No queries at all.
        # WHY: Edge case -- nothing to evaluate.
        # EXPECTED: Default on_empty="fail" rejects empty query sets.
        with pytest.raises(MltkAssertionError) as exc_info:
            assert_ndcg([], [], k=5, min_ndcg=0.8)
        assert "no queries provided" in exc_info.value.result.message.lower()

    def test_empty_queries_skip(self) -> None:
        # SCENARIO: Caller explicitly chooses to skip empty query sets.
        # EXPECTED: Passes as INFO with skip metadata.
        result = assert_ndcg([], [], k=5, min_ndcg=0.8, on_empty="skip")
        assert result.passed is True
    ...
    def test_empty_queries_pass_legacy(self) -> None:
        # SCENARIO: Caller explicitly chooses legacy empty-query behavior.
    ...
    def test_all_zero_relevance_fails_by_default(self) -> None:
        # SCENARIO: All documents have relevance 0.
        # WHY: IDCG is 0 for every query, so no query can be evaluated.
        # EXPECTED: Default on_empty="fail" rejects the all-empty mean.
        y_true = [[0, 0, 0]]
        y_scores = [[0.5, 0.3, 0.1]]
        with pytest.raises(MltkAssertionError) as exc_info:
            assert_ndcg(y_true, y_scores, k=3, min_ndcg=0.5)
        assert "no queries with relevant documents" in (
            exc_info.value.result.message.lower()
        )

    def test_zero_relevance_queries_excluded_from_ndcg_mean(self) -> None:
        # SCENARIO: One query has zero IDCG, another has relevant documents.
        # EXPECTED: The zero-IDCG query is excluded from the mean.
        y_true = [[0, 0, 0], [1, 0, 0]]
        y_scores = [[0.5, 0.3, 0.1], [0.9, 0.2, 0.1]]
        result = assert_ndcg(y_true, y_scores, k=3, min_ndcg=1.0)
        assert result.passed is True
        assert result.details["num_queries"] == 1
        assert result.details["per_query_ndcg"] == [1.0]
...
class TestMRR:
    ...
    def test_no_relevant_results_fail_by_default(self) -> None:
        # SCENARIO: No query has any relevant result.
        # WHY: No query has a first relevant result to rank.
        # EXPECTED: Default on_empty="fail" rejects the all-empty mean.
    ...
    def test_empty_queries_fail_by_default(self) -> None:
        ...
        # EXPECTED: Default on_empty="fail" rejects empty query sets.
        with pytest.raises(MltkAssertionError) as exc_info:
            assert_mrr([], min_mrr=0.5)
        assert "no queries provided" in exc_info.value.result.message.lower()

    def test_queries_without_relevant_results_excluded_from_mrr_mean(self) -> None:
        # SCENARIO: One query has no relevant result; one has a first hit.
        # EXPECTED: The no-relevance query is excluded from the mean.
        result = assert_mrr([[False, False], [True, False]], min_mrr=1.0)
        assert result.passed is True
        assert result.details["num_queries"] == 1
        assert result.details["per_query_rr"] == [1.0]
...
class TestRecallAtK:
    ...
    def test_empty_queries_fail_by_default(self) -> None:
        ...
        # EXPECTED: Default on_empty="fail" rejects empty query sets.
        with pytest.raises(MltkAssertionError) as exc_info:
            assert_recall_at_k([], [], k=5, min_recall=0.8)
        assert "no queries provided" in exc_info.value.result.message.lower()

    def test_empty_relevant_set_for_query_excluded_from_mean(self) -> None:
        # SCENARIO: A query has no relevant documents defined.
        # WHY: Recall is undefined for that query.
        # EXPECTED: That query is excluded from the mean.
        relevant = [set(), {"d1"}]
        retrieved = [["d5"], ["d1"]]
        result = assert_recall_at_k(
            relevant, retrieved, k=5, min_recall=1.0,
    ...
    def test_all_empty_relevant_sets_fail_by_default(self) -> None:
        # SCENARIO: Every query has no relevant documents defined.
        # EXPECTED: Default on_empty="fail" rejects the all-empty mean.
        with pytest.raises(MltkAssertionError) as exc_info:
            assert_recall_at_k([set(), set()], [["a"], ["b"]], k=5)
        assert "no queries with relevant documents" in (
            exc_info.value.result.message.lower()
        )
...
class TestMAPAtK:
    ...
    def test_empty_queries_fail_by_default(self) -> None:
        ...
        # EXPECTED: Default on_empty="fail" rejects empty query sets.
        with pytest.raises(MltkAssertionError) as exc_info:
            assert_map_at_k([], [], k=5, min_map=0.5)
        assert "no queries provided" in exc_info.value.result.message.lower()

    def test_empty_relevant_set_for_query_excluded_from_map_mean(self) -> None:
        # SCENARIO: A query has no relevant documents defined.
        # EXPECTED: The empty query is excluded from MAP.
        result = assert_map_at_k(
            [set(), {"d1"}], [["d5"], ["d1"]], k=5, min_map=1.0
        )
        assert result.passed is True
        assert result.details["per_query_ap"] == [1.0]
        assert result.details["num_queries"] == 1
...
class TestRetrievalPerformance:
    ...
    def test_100_queries_ndcg(self) -> None:
        ...
        assert 0 < result.details["num_queries"] <= 100
