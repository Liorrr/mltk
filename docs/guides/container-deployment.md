...
mltk ships two officially supported Docker images published simultaneously to
**Docker Hub** and GitHub Container Registry (GHCR). Both are
multi-architecture (`linux/amd64` and `linux/arm64`) and are built from the
root `Dockerfile` in this repository.

| Image | Base | Extra tooling | Use case |
| --- | --- | --- | --- |
| `liorrr/mltk:latest` | `python:3.12-slim` + `mltk[all]` | none | General CLI, scans, MCP server, CI jobs |
| `liorrr/mltk:full` | `latest` + Trivy 0.60.0 | `/usr/local/bin/trivy` | Container security scanning |

Version-pinned tags (`liorrr/mltk:v0.12.0`, `...:v0.12.0-full`) are published
automatically on every `v*` git tag via the `docker-publish.yml` GitHub
Actions workflow.
...
Docker Hub is the default registry — no URL prefix needed:

```bash
# Slim runtime (default — the one you want 90% of the time)
docker pull liorrr/mltk:latest

# Full runtime with Trivy bundled for container scanning
docker pull liorrr/mltk:full
```

The same images are also available on GHCR if you prefer:
...
```bash
docker run --rm liorrr/mltk:latest --help
docker run --rm liorrr/mltk:latest --version
```

The image's `ENTRYPOINT` is `mltk`, so any CLI subcommand works directly:

```bash
docker run --rm -v "$PWD":/work -w /work liorrr/mltk:latest \
...
```bash
docker run --rm -e TRIVY_BIN=/opt/trivy/bin/trivy liorrr/mltk:full ...
```
...
```bash
docker run --rm liorrr/mltk:full trivy --version
```
