...
### Stats (30 sec)
> "232 assertions covering the full ML lifecycle. Data quality, model validation, behavioral consistency, RAG evaluation, red team security, compliance, production monitoring. Native pytest integration -- it's a plugin, not a separate tool. One pip install. Today I'll show you what that looks like in practice."
...
> "232 assertions. One pip install. Native pytest. You don't learn a new tool -- you write `assert_paraphrase_invariance` the same way you write `assert True`. It runs in CI, it runs locally, it runs in your existing test suite."
...
> "Three things to remember. First: **breadth**. 232 assertions covering the full ML lifecycle. No other tool does data quality AND model testing AND LLM evaluation AND security AND compliance in one package."
...
OPENING: "pytest catches code bugs. mltk catches ML bugs."
         232 assertions, native pytest, one pip install.
...
BEAT 6:  "232 assertions. One pip install. pytest for ML."
