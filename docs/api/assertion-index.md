...
Complete index of all assertion functions in mltk, organized by category.
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 11 | `assert_no_pii` | `mltk.data.pii` | Scan DataFrame columns for PII patterns (email, phone, SSN, credit card, API keys, and 30+ more) | v0.1.0 |

### Labels

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 12 | `assert_label_balance` | `mltk.data.labels` | Verify label class ratio does not exceed imbalance threshold | v0.1.0 |
| 13 | `assert_label_coverage` | `mltk.data.labels` | Verify all expected labels are present with minimum sample count | v0.1.0 |

### Statistics

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 14 | `assert_column_mean` | `mltk.data.statistics` | Verify column mean falls within [min, max] bounds | v0.5.0 |
| 15 | `assert_column_median` | `mltk.data.statistics` | Verify column median falls within [min, max] bounds | v0.5.0 |
| 16 | `assert_column_stdev` | `mltk.data.statistics` | Verify column standard deviation falls within [min, max] bounds | v0.5.0 |
| 17 | `assert_quantiles` | `mltk.data.statistics` | Verify column quantile values fall within expected bounds | v0.5.0 |

### Validation

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 18 | `assert_datetime_format` | `mltk.data.validation` | Verify all values in a column parse as valid datetime with given format | v0.5.0 |
| 19 | `assert_values_in_set` | `mltk.data.validation` | Verify all column values belong to an allowed set | v0.5.0 |
| 20 | `assert_no_conflicting_labels` | `mltk.data.validation` | Detect rows with identical features but different labels | v0.5.0 |
| 21 | `assert_feature_label_correlation_stable` | `mltk.data.validation` | Verify feature-label correlations have not shifted between train and production | v0.6.0 |

### Data Lineage

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 22 | `assert_lineage_complete` | `mltk.data.lineage` | Verify all required pipeline steps appear in the lineage graph | v0.1.0 |

### Preset (One-Call Bundle)

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 23 | `assert_data_quality` | `mltk.data.preset` | Run schema, nulls, range, outliers, and PII checks in a single call; returns TestSuite | v0.5.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 24 | `assert_metric` | `mltk.model.metrics` | Verify any sklearn metric (accuracy, precision, recall, F1, ROC-AUC, etc.) meets threshold | v0.1.0 |

### Regression

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 25 | `assert_no_regression` | `mltk.model.regression` | Fail if current model metric drops below baseline by more than tolerance | v0.1.0 |

### Slicing

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 26 | `assert_slice_performance` | `mltk.model.slicing` | Verify metric holds across data slices (subgroups) | v0.1.0 |
| 27 | `assert_calibration` | `mltk.model.slicing` | Verify predicted probabilities match observed frequencies | v0.1.0 |

### Bias

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 28 | `assert_no_bias` | `mltk.model.bias` | Detect bias across groups using 5 fairness methods (demographic parity, equal opportunity, predictive parity, calibration, individual fairness) | v0.1.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 30 | `assert_no_overfitting` | `mltk.model.overfitting` | Fail if train-test score gap exceeds threshold | v0.5.0 |
| 31 | `assert_label_drift` | `mltk.model.overfitting` | Detect label distribution drift between train and serving | v0.5.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 32 | `assert_no_train_test_overlap` | `mltk.training.leakage` | Fail if train and test DataFrames share any rows | v0.1.0 |
| 33 | `assert_temporal_split` | `mltk.training.leakage` | Verify all train dates precede all test dates | v0.1.0 |
| 34 | `assert_no_target_leakage` | `mltk.training.leakage` | Detect features with suspiciously high correlation to target | v0.1.0 |

### Gradient Health

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 35 | `assert_gradient_flow` | `mltk.training.gradient` | Verify gradients are non-zero across layers (no dead layers) | v0.2.0 |
| 36 | `assert_no_vanishing_gradient` | `mltk.training.gradient` | Fail if gradient norms drop below threshold | v0.2.0 |
| 37 | `assert_no_exploding_gradient` | `mltk.training.gradient` | Fail if gradient norms exceed max threshold | v0.2.0 |
| 38 | `assert_loss_finite` | `mltk.training.gradient` | Verify all loss values are finite (no NaN/Inf) | v0.2.0 |

### Numerical Stability

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 39 | `assert_no_nan_inf` | `mltk.training.numerical` | Verify arrays contain no NaN or Inf values | v0.2.0 |
| 40 | `assert_loss_decreasing` | `mltk.training.numerical` | Verify training loss trend is decreasing over time | v0.2.0 |
| 41 | `assert_no_loss_divergence` | `mltk.training.numerical` | Fail if loss values diverge (spike beyond threshold) | v0.2.0 |
| 42 | `assert_softmax_valid` | `mltk.training.numerical` | Verify softmax output rows sum to 1.0 with no negatives | v0.2.0 |

### Augmentation

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 43 | `assert_no_augmentation_on_test` | `mltk.training.augmentation` | Verify test data has not been augmented (near-duplicate detection) | v0.3.0 |
| 44 | `assert_augmentation_preserves_signal` | `mltk.training.augmentation` | Verify augmentation does not alter label distribution | v0.3.0 |

### Checkpoint

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 45 | `assert_checkpoint_complete` | `mltk.training.checkpoint` | Verify checkpoint file exists and contains all required keys | v0.3.0 |
| 46 | `assert_resume_loss_continuous` | `mltk.training.checkpoint` | Verify loss continuity when resuming from checkpoint | v0.3.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 49 | `assert_no_memory_leak` | `mltk.training.memory` | Detect monotonically increasing memory usage over training steps | v0.3.0 |
| 50 | `assert_loss_is_detached` | `mltk.training.memory` | Verify loss tensor is properly detached (no graph retention leak) | v0.3.0 |

### Training-Serving Skew

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 51 | `assert_no_training_serving_skew` | `mltk.training.skew` | Verify train and serving outputs are numerically close | v0.5.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 52 | `assert_latency` | `mltk.inference.latency` | Verify function latency P50/P95/P99 meets threshold (with warmup exclusion) | v0.1.0 |
| 53 | `assert_cold_start` | `mltk.inference.latency` | Verify first-call latency meets cold start threshold | v0.1.0 |
| 54 | `assert_throughput` | `mltk.inference.throughput` | Verify requests-per-second meets minimum (sequential or concurrent) | v0.1.0 |
| 55 | `assert_api_contract` | `mltk.inference.contract` | Verify function input/output schemas match JSON Schema contract | v0.1.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 56 | `assert_iou` | `mltk.domains.cv.detection` | Verify mean IoU between predicted and ground-truth bounding boxes | v0.1.0 |
| 57 | `assert_map` | `mltk.domains.cv.detection` | Verify mean Average Precision for object detection | v0.1.0 |
| 58 | `assert_frame_accuracy` | `mltk.domains.cv.video` | Verify per-frame classification accuracy for video | v0.1.0 |
| 59 | `assert_temporal_consistency` | `mltk.domains.cv.video` | Verify smooth bounding box tracking across video frames | v0.1.0 |
| 60 | `assert_topk_accuracy` | `mltk.domains.cv.classification` | Verify top-K classification accuracy | v0.1.0 |
| 61 | `assert_face_far` | `mltk.domains.cv.face` | Verify False Accept Rate for face recognition systems | v0.1.0 |
| 62 | `assert_mota` | `mltk.domains.cv.tracking` | Verify Multiple Object Tracking Accuracy (misses, FP, ID switches) | v0.2.0 |
| 63 | `assert_motp` | `mltk.domains.cv.tracking` | Verify Multiple Object Tracking Precision (localization quality) | v0.2.0 |
| 64 | `assert_idf1` | `mltk.domains.cv.tracking` | Verify IDF1 score for identity-aware tracking consistency | v0.2.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 65 | `assert_bleu` | `mltk.domains.nlp.generation` | Verify BLEU score for text generation quality | v0.1.0 |
| 66 | `assert_rouge` | `mltk.domains.nlp.generation` | Verify ROUGE score (rouge1/rouge2/rougeL) for summarization | v0.1.0 |
| 67 | `assert_ner_f1` | `mltk.domains.nlp.ner` | Verify named entity recognition F1 score | v0.1.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 71 | `assert_wer` | `mltk.domains.speech.recognition` | Verify Word Error Rate is below threshold | v0.1.0 |
| 72 | `assert_cer` | `mltk.domains.speech.recognition` | Verify Character Error Rate is below threshold | v0.1.0 |
| 73 | `assert_rtf` | `mltk.domains.speech.performance` | Verify Real-Time Factor (processing speed vs audio duration) | v0.1.0 |
| 74 | `assert_accent_coverage` | `mltk.domains.speech.performance` | Verify WER gap across accents stays below bias threshold | v0.1.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 78 | `assert_ttft` | `mltk.domains.llm.latency` | Verify Time To First Token meets latency threshold | v0.1.0 |
| 79 | `assert_itl` | `mltk.domains.llm.latency` | Verify Inter-Token Latency meets threshold | v0.1.0 |

### RAG Evaluation

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 80 | `assert_faithfulness` | `mltk.domains.llm.rag` | Verify answer is grounded in retrieved context (no hallucination) | v0.5.0 |
| 81 | `assert_context_relevancy` | `mltk.domains.llm.rag` | Verify retrieved context is relevant to the question | v0.5.0 |
| 82 | `assert_answer_relevancy` | `mltk.domains.llm.rag` | Verify answer is relevant to the original question | v0.5.0 |
| 83 | `assert_context_precision` | `mltk.domains.llm.rag` | Verify relevant documents are ranked higher in context | v0.5.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 86 | `assert_coherence` | `mltk.domains.llm.coherence` | Verify sentence-to-sentence consistency within generated text | v0.6.0 |
| 87 | `assert_bertscore` | `mltk.domains.llm.bertscore` | Verify BERTScore F1 (embedding-based precision/recall) meets threshold | v0.6.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 91 | `assert_text_length` | `mltk.domains.llm.text_quality` | Verify generated text length falls within [min, max] word bounds | v0.5.0 |
| 92 | `assert_output_format` | `mltk.domains.llm.text_quality` | Verify text matches a regex pattern (JSON, UUID, date, etc.) | v0.5.0 |
| 93 | `assert_readability` | `mltk.domains.llm.text_quality` | Verify text readability score (Flesch-Kincaid grade level) | v0.5.0 |

### Multi-Turn Conversation

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 94 | `assert_knowledge_retention` | `mltk.domains.llm.conversation` | Verify assistant retains information across conversation turns | v0.5.0 |
| 95 | `assert_turn_relevancy` | `mltk.domains.llm.conversation` | Verify each assistant response is relevant to the preceding user turn | v0.5.0 |
| 96 | `assert_conversation_completeness` | `mltk.domains.llm.conversation` | Verify conversation covers all expected topics | v0.5.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 100 | `assert_endpoint_healthy` | `mltk.monitor.aws` | Verify SageMaker endpoint status is InService | v0.3.0 |
| 101 | `assert_endpoint_latency` | `mltk.monitor.aws` | Verify SageMaker endpoint invocation latency from CloudWatch | v0.3.0 |
| 102 | `assert_endpoint_error_rate` | `mltk.monitor.aws` | Verify SageMaker endpoint error rate from CloudWatch | v0.3.0 |

### GCP Vertex AI

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 103 | `assert_endpoint_healthy` | `mltk.monitor.gcp` | Verify Vertex AI endpoint is deployed and serving | v0.3.0 |
| 104 | `assert_prediction_latency` | `mltk.monitor.gcp` | Verify Vertex AI prediction latency from Cloud Monitoring | v0.3.0 |

### Azure ML

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 105 | `assert_endpoint_healthy` | `mltk.monitor.azure` | Verify Azure ML managed endpoint is healthy | v0.3.0 |
| 106 | `assert_endpoint_latency` | `mltk.monitor.azure` | Verify Azure ML endpoint response latency | v0.3.0 |

### Prometheus / Triton

| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 107 | `assert_prometheus_metric` | `mltk.monitor.prometheus` | Query Prometheus and verify a metric meets threshold | v0.3.0 |
| 108 | `assert_gpu_utilization` | `mltk.monitor.prometheus` | Verify GPU utilization from Prometheus DCGM exporter | v0.3.0 |
| 109 | `assert_triton_healthy` | `mltk.monitor.prometheus` | Verify NVIDIA Triton Inference Server is ready | v0.3.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 110 | `assert_reproducible` | `mltk.pipeline.reproducibility` | Verify function produces identical output across multiple runs | v0.1.0 |
| 111 | `assert_checksum` | `mltk.pipeline.reproducibility` | Verify file SHA-256 checksum matches expected value | v0.1.0 |
| 112 | `assert_pipeline` | `mltk.pipeline.e2e` | Run a sequence of pipeline steps and verify all succeed | v0.1.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 114 | `assert_feature_drift` | `mltk.domains.tabular.features` | Detect per-column drift across two DataFrames | v0.1.0 |
| 115 | `assert_feature_importance_stable` | `mltk.domains.tabular.features` | Verify SHAP feature importance ranking stability (WARNING severity) | v0.1.0 |
| 116 | `assert_class_balance` | `mltk.domains.tabular.quality` | Convenience wrapper for label balance check on DataFrame column | v0.1.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 117 | `assert_matches_golden` | `mltk.testing.golden` | Verify current output matches a versioned golden baseline (dict, list, or numpy array) | v0.4.0 |
...
| # | Assertion | Module | Description | Since |
|---|-----------|--------|-------------|-------|
| 118 | `assert_true` | `mltk.core.assertion` | Primitive assertion — wrap any boolean condition into a TestResult | v0.1.0 |
...
from mltk.training.distributed import assert_effective_batch_size, assert_gradient_sync
...
from mltk.domains.llm.safety import assert_no_toxicity, assert_no_hallucination
...
from mltk.domains.llm.agentic import assert_task_completion, assert_tool_selection, assert_tool_call_correctness
